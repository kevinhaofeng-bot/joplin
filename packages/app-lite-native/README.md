...
- 卡片列表只保留轻量笔记投影；搜索、更新排序和选中状态按笔记 ID 维护，首图缩略图按可见卡片异步下采样和有界缓存。
- 标题与正文独立编辑，支持中文、emoji、输入法组合文本、撤销/重做和延迟自动保存；保存失败会保留可见内容并允许重试。
- 语义富文本支持正文、H1/H2/H3、粗体、斜体、下划线、高亮、删除线、链接、对齐、缩进、项目符号、编号列表和清单。
- 图片支持 PNG/JPEG 像素剪贴板、TIFF 剪贴板（规范化为 PNG）和 Finder PNG/JPEG 文件拖入；缺失资源保持可恢复的语义锚点，正文不存储图片 bytes 或 base64。
