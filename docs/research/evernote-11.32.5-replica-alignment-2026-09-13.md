...
本页以 `codex/joplin-lite-native-rust` 的 v0.21 检查点为基线，并跟踪其后已通过限界复审的 ENEX C1b 只读扫描、C2a 纯内容转换、C2b 隔离暂存、JEX C2c-1 原始资料暂存及 C2c-2a 纯 Markdown 转换。它更新
...
| Joplin 归档迁移 | Evernote `36364` 导入流程仅提供解析与 mutation 分离的行为参考 | C2c-1 `cd8632cfd..4c059b889` 对所有 JEX 原件做字节哈希、二次 tar 核验和独立临时 SQLite/流式附件暂存；C2c-2a `7065d360e..40e7fa044` 对受限 Markdown 作纯规范文档转换、资源映射与保真阻断，最终独立复审 0 Critical/Important、控制者 core 186/186。JEX 格式细节来自 Joplin `InteropService_Importer_Jex/Raw`、`BaseItem` 与 `MarkupToHtml`/`MdToHtml` | **仍没有 JEX 导入**：原件暂存和纯转换都不是可打开的笔记库；HTML 正文、笔记本/标签/关系、索引、正式切换、可读导出/恢复及真实用户归档核验均待完成。不能将 Joplin 源码的语法细节称为 Evernote 逆向成果 |
...
Task 8 的 C2b 已完成 ENEX **隔离暂存**：保留 ENML 审计原文、逐篇只用同笔记的已验证附件转换，遇不支持结构返回具路径的错误，失败不返回可发布句柄；正式资料库完全未进入此轮。C2c-1 完成 JEX 原始资料的独立、可重读暂存；C2c-2a 完成受限 Markdown 的纯保真转换，HTML 仍有明确阻断。二者都尚未将正文或关系导入规范笔记库。下一段做 HTML 结构预验证/转换，再做 JEX 隔离笔记库暂存、正式资料库切换与可读导出/恢复。同步从 Task 9 独立推进，不能混入导入事务。
