...
本页以 `codex/joplin-lite-native-rust` 的 v0.21 检查点为基线，并跟踪其后已通过限界复审的 ENEX C1b 只读扫描、C2a 纯内容转换、C2b 隔离暂存、JEX C2c-1 原始资料暂存、C2c-2a/2b 纯正文转换及 C2c-3a 无附件笔记的隔离资料库。它更新
...
| Joplin 归档迁移 | Evernote `36364` 导入流程仅提供解析与 mutation 分离的行为参考 | C2c-1 `cd8632cfd..4c059b889` 对 JEX 原件做字节哈希、二次 tar 核验和独立原件暂存；C2c-2a/2b `7065d360e..057b700ba` 对受限 Markdown 与严格 XHTML-like HTML 作纯保真转换；C2c-3a `73f02f512` 已将无附件/组织关系的 JEX 笔记写成可重开的隔离 SQLite 资料库，核对原文、时间、正文搜索与零迁移 outbox。独立复审 0 Critical/Important，控制者 core 194/194。JEX 格式细节来自 Joplin `InteropService_Importer_Jex/Raw`、`BaseItem` 与 renderer | **只有 note-only JEX 隔离导入**：遇资源、笔记本、标签或关系会明确阻断；完整 HTML5/插件保真、正式切换、可读导出/恢复及真实个人归档核验均待完成。不能将 Joplin 源码语法细节称为 Evernote 逆向成果 |
...
Task 8 的 C2b 已完成 ENEX **隔离暂存**：保留 ENML 审计原文、逐篇只用同笔记的已验证附件转换，遇不支持结构返回具路径的错误，失败不返回可发布句柄；正式资料库完全未进入此轮。C2c-1 完成 JEX 原件暂存，C2c-2a/2b 完成受限 Markdown/HTML 的纯转换，C2c-3a 已能导入无附件/组织关系的 JEX 笔记至新建临时资料库。下一段补资源/关联与笔记本/标签，再做正式资料库切换及可读导出/恢复。同步从 Task 9 独立推进，不能混入导入事务。
