...
本页以 `codex/joplin-lite-native-rust` 的 v0.21 检查点为基线，并跟踪其后已通过限界复审的 ENEX C1b 只读扫描、C2a 纯内容转换、C2b 隔离暂存、JEX C2c-1 原始资料暂存、C2c-2a/2b 纯正文转换、C2c-3a/3b/3c-1/3c-2 隔离资料库及 C2c-4a 只读资格检查。它更新
...
| Joplin 归档迁移 | Evernote `36364` 导入流程仅提供解析与 mutation 分离的行为参考 | C2c-1 `cd8632cfd..4c059b889` 对 JEX 原件做字节哈希、二次 tar 核验和原件暂存；C2c-2a/2b `7065d360e..057b700ba` 对受限 Markdown 与严格 XHTML-like HTML 作纯保真转换；C2c-3a/3b `73f02f512..ef6e45c25` 已将含图片/PDF/重复引用的 JEX 笔记写成可重开的隔离 SQLite＋blob 资料库；C2c-3c-1/2 `dab1b920a..7232faaf0` 增加受限两级文件夹、所属笔记、标签和 note-tag 关联，重开核对原文、时间、资源、搜索及零迁移 outbox。C2c-4a `f0eda7aa6..90327835c` 增加 exporter-shaped 只读资格分类；独立复审 0 Critical/Important/Minor，控制者 core 212/212。对现成 1.1 GB JEX 备份实测：1,665 笔记/31 文件夹/64 标签/425 关联/4,153 资源，33.65 秒、峰值 RSS 23,035,904 字节。JEX 格式细节来自 Joplin Raw importer、exporter、Resource 和 renderer | **真实 JEX 尚不能视为可迁**：预检已发现 1 个未导出内部引用和 1 张超出当前 10 MiB 图片 Store 限额的 PNG，语义扫描因此未完成；Joplin `BaseItem.serialize` 全字段与当前窄 stage 仍有兼容差距。现用库还有待办、来源 URL、自定义排序、4 篇普通 JEX 未导出的已删除笔记及 1 个含 187 篇直属笔记的混合父文件夹，均需保真策略。正式切换、可读导出/恢复和真实个人归档验收待完成；Joplin 语法不能称为 Evernote 逆向成果 |
