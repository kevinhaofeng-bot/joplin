...
本页以 `codex/joplin-lite-native-rust` 的 v0.21 检查点为基线，并跟踪其后已通过限界复审的 ENEX C1b 只读扫描、C2a 纯内容转换和 C2b 隔离暂存。它更新
...
Task 8 的 C2b 已完成 ENEX **隔离暂存**：保留 ENML 审计原文、逐篇只用同笔记的已验证附件转换，遇不支持结构返回具路径的错误，失败不返回可发布句柄；正式资料库完全未进入此轮。下一段要实现 Joplin JEX 的相同安全边界，再处理正式资料库切换与可读导出/恢复。同步从 Task 9 独立推进，不能混入导入事务。
