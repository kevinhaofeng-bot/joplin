...
本页以 `codex/joplin-lite-native-rust` 的 v0.21 检查点为基线，并跟踪其后已通过限界复审的 ENEX C1b 只读扫描、C2a 纯内容转换、C2b 隔离暂存与 JEX C2c-1 原始资料暂存。它更新
...
| Joplin 归档迁移 | Evernote `36364` 导入流程仅提供解析与 mutation 分离的行为参考 | v0.20/v0.21 的 `scan_jex_archive` 读 Joplin Raw/JEX；C2c-1 `cd8632cfd..4c059b889` 用所有条目的原始字节哈希、二次 tar 核验、独立临时 SQLite 原件库及流式附件，重开校验完整性，截断资源拒绝且无暂存残片。依据 Joplin `InteropService_Importer_Jex/Raw`、`BaseItem.unserialize`、`Resource.filename` 和 renderer `MarkupLanguage`，独立复审 0 Critical/Important，控制者 core 176/176 | **仍没有 JEX 导入**：原件暂存不是可打开的笔记库；Markdown/HTML 转规范文档、笔记本/标签/关系、索引、正式切换、可读导出/恢复及真实用户归档核验均待完成。JEX 格式细节来自 Joplin 源码，不得称为 Evernote 逆向成果 |
...
Task 8 的 C2b 已完成 ENEX **隔离暂存**：保留 ENML 审计原文、逐篇只用同笔记的已验证附件转换，遇不支持结构返回具路径的错误，失败不返回可发布句柄；正式资料库完全未进入此轮。C2c-1 仅完成 JEX 原始资料的独立、可重读暂存，尚未将正文或关系导入规范笔记库。下一段先做严格的 Markdown/HTML 正文转换，再做 JEX 隔离笔记库暂存、正式资料库切换与可读导出/恢复。同步从 Task 9 独立推进，不能混入导入事务。
