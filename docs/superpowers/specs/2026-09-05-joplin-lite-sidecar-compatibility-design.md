...
- stdin 和 stdout 使用 UTF-8 NDJSON，一行一个完整 JSON frame。
- stdout 只允许协议 frame；诊断只写 stderr。
- 每个输入 frame 最大 `8 MiB`。资源二进制永远不通过该协议内联传输。
- 一旦收到非法 UTF-8、超长 frame 或无法解析的 JSON，sidecar 返回可关联时尽量关联的稳定错误；无法安全继续分帧时退出。
...
- 非 32 位十六进制 ID 和包含路径穿越的资源扩展名被拒绝；
...
- 单个响应 frame 超过 `8 MiB` 立即终止子进程并进入 `Failed`。
- `Drop` 和显式关闭都要回收子进程；显式关闭优先发送 `shutdown`，到期后再 kill。
