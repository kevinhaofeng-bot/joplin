...
2026-09-07 对用户当前 Joplin profile 做只读聚合统计，不读取或复制正文内容：1,664 篇有效笔记中，1,150 篇（69.1%）来自 Evernote，513 篇（30.8%）在 Joplin 新建。Evernote 来源的最后创建时间为 2020-04-20，Joplin 来源从 2020-04-22 开始；尽管后者使用时间更长，数量仍不足前者一半。

两组内容形态差异同样明显：Evernote 导入笔记平均正文约 2,401 字符，390 篇超过 1,000 字；Joplin 新建平均约 1,281 字符，80 篇超过 1,000 字。Evernote 组 598/1,150 篇含资源、共 4,027 个资源引用；Joplin 组 73/513 篇含资源、共 338 个资源引用。即使扣除 Evernote 网页剪藏和邮件来源，桌面/手机来源仍约 989 篇。

这组数据把产品目标从“功能存在”提高为“恢复记录意愿”：新建和持续输入必须低摩擦；列表必须可凭标题、摘要和缩略图快速扫读；图片与附件必须是正文一等结构；搜索和同步状态必须让用户信任。后续真实使用评估持续观察新建频率、长笔记比例、带资源笔记比例和旧内容再次打开率，但不把诱导用户增加数量作为目标。
...
格式服务于用户和产品，不是产品目标本身。Joplin Lite 不追求“纯 Markdown”，也不把全文本当作洁癖：选择语义 HTML 是因为它在当前阶段最适合完整富文本、原生所见即所得、可靠索引、Joplin 互通和可控迁移。如果未来另一种有版本的结构化表示能更好地满足这些目标，可以在保真迁移后演进；不得为了扩展名、格式纯度或实现方便牺牲编辑体验和内容完整性。Markdown 只作为导入、导出与可选输入捷径。
