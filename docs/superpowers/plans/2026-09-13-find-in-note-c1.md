...
**Spec:** `.superpowers/sdd/2026-09-11-evernote-core-notes-roadmap/task-7-source-brief.md` (approved Task 7 find rows) and this bounded C1 design. Original Evernote 11.32.5 source is under `/Users/kevinhao/Projects/joplin-reconstruction/evernote-11.32.5/common-editor-sourcemap/@evernote/common-editor/src/apps/peso/modules/find/`: `commands/find.ts`, `findnext.ts`, `findprev.ts`, `closeFindInNote.ts`, `state.ts`, `ui/findInNoteUiPlugin.tsx`, `ui/FindInNote.tsx`, `ui/keyBindings.ts`, `utils/index.ts`. Read the exact files when implementing; the source-backed behavior is ephemeral document-search state, literal in-note substring grammar distinct from library search, circular next/previous, close-UI preserving highlights, a shell-owned overlay, and viewport-bounded decorations. This is behavioral reconstruction, not a port of Electron/React code.
