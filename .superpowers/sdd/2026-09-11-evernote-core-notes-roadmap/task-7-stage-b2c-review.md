...
## Fix round 1 scoped re-review — `332e77002..a2559a14c`

Independent verdict: all three Important findings **ADDRESSED**, no new Critical/Important breakage. History `Ok(false)` re-checks and re-schedules only a still-pending same-direction target (`ui/mod.rs:3674`); Retry checks the current target/pending status before showing progress (`ui/mod.rs:5853`), with a real injected history persist failure→Forward Retry click→restore mounted test (`ui/tests.rs:1114`); Link popover focus/visibility and retained session/selection/undo now survive Escape and actual backdrop click (`ui/tests.rs:486`), with search backdrop pointer occlusion (`ui/mod.rs:4997`). `toolbar_more` is a mouse-only div, not a captured focus owner (`ui/mod.rs:5371`). Minor test breadth remaining: explicit stale `Ok(false)`, Back-direction Retry, and vanished-target Retry fault injections. Controller separately ran final-head GPUI suite 1296/0/1; Release smoke remained pending at re-review time. Reviewer did not edit files or run the whole suite.
